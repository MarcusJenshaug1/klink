...
import type { Card, Pack, Intensitet } from '@/types/game'
...
export function GameCard({ card, pack, players, intensitet, onNext }: GameCardProps) {
  const meta = CARD_TYPE_META[card.type]
  const sips = useMemo(() => getSips(intensitet), [card.id, intensitet])

  const content = useMemo(() => {
    let text = interpolate(card.innhold, players)
    text = replaceSips(text, sips)
    return text
  }, [card.id, players, sips])
  ...
  return (
    <div
      className="absolute inset-0 flex flex-col items-center justify-center px-5 landscape:px-12"
      style={{ backgroundColor: pack.farge }}
    >
      {/* The card */}
      <div className="w-full max-w-sm landscape:max-w-2xl flex flex-col gap-4">

        {/* Category badge — above card */}
        <div className="flex justify-center">
          <span className="inline-flex items-center gap-1.5 px-4 py-1.5 rounded-full bg-black/20 backdrop-blur-sm text-white/90 text-xs font-black uppercase tracking-widest">
        ...
        <div className="bg-white/15 backdrop-blur-md rounded-3xl p-7 flex flex-col gap-5 shadow-xl">

          {/* Card text */}
          <p className="text-white text-xl sm:text-2xl font-semibold leading-relaxed text-center">
            {content}
          </p>
          ...
          {hasTimer && (
            <div className="flex flex-col items-center gap-3">
              ...
              {timerPhase === 'idle' && (
                <button
                  onClick={handleStart}
                  className="w-full flex items-center justify-center gap-2 bg-white/20 hover:bg-white/30 active:scale-95 text-white font-black text-base py-3.5 rounded-2xl transition-all"
                >
              ...
              {timerPhase === 'running' && (
                <div className="flex flex-col items-center gap-3 w-full">
                  {timerSynlig && (
                    <p className="text-white font-black tabular-nums text-center" style={{ fontSize: '5rem', lineHeight: 1 }}>
                      {countdownDisplay}
                    </p>
                  )}
                  {!timerSynlig && (
                    <button
                      onClick={handleStop}
                      className="w-full flex items-center justify-center gap-2 bg-red-500/80 hover:bg-red-500 active:scale-95 text-white font-black text-base py-3.5 rounded-2xl animate-pulse transition-all"
                    >
                      <Timer className="w-5 h-5" />
                      Stopp!
                    </button>
