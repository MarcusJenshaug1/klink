...
// Spinning delays in ms — starts fast, slows to a dramatic stop
const SPIN_DELAYS = [60, 60, 70, 70, 80, 100, 130, 170, 220, 300, 420, 600, 800]
...
export function TrekkingCard({ card, pack, players, intensitet, korttyper, onNext }: TrekkingCardProps) {
  ...
  const winner = players.length > 0 ? players[seedIndex(card.id, players.length)] : null
  ...
  const startSpin = () => {
    if (spinning || done || players.length === 0) return
    setSpinning(true)

    const winnerIdx = seedIndex(card.id, players.length)
    let step = 0
    let current = 0
    ...
    const tick = () => {
      ...
      if (step < SPIN_DELAYS.length) {
        setTimeout(tick, SPIN_DELAYS[step])
      } else {
        // Force land on winner
      ...
      <div className="flex min-h-0 w-full max-w-sm flex-col items-center gap-3 md:max-w-xl md:gap-5 lg:max-w-2xl xl:max-w-3xl landscape:max-w-2xl landscape:gap-2 lg:landscape:max-w-3xl">
        ...
        >
          ...
          <div className="relative z-10 flex flex-col items-center gap-5 md:gap-7 landscape:gap-4">
            ...
            {/* Card text */}
            {card.innhold && (
              <p className="break-words text-center text-xl font-semibold leading-snug text-white sm:text-2xl landscape:text-lg [overflow-wrap:anywhere]">
                {segments.map((seg, i) =>
                  seg.type === 'player' ? (
            ...
            <div
              className={cn(
                'flex w-full items-center justify-center rounded-2xl border px-6 py-5 landscape:py-3',
                done
                  ? athina ? 'border-white/50 bg-white/25' : 'border-white/50 bg-white/25'
                  : 'border-white/20 bg-black/20'
              )}
              style={done ? { borderColor: colorWithAlpha(accent, 0.6, 'rgba(255,255,255,0.5)') } : undefined}
            >
              <span className={cn(
                'truncate text-center font-black text-white transition-all',
                done ? 'text-3xl sm:text-4xl landscape:text-2xl' : 'text-2xl sm:text-3xl landscape:text-xl opacity-80'
              )}>
                {spinning || done
                  ? players[displayIdx] ?? '?'
                  : players.length > 0 ? '🎲' : 'Ingen spillere'
                }
              </span>
            </div>
            ...
            {!done ? (
              <button
                ...
                className={cn(primaryActionClass, spinning && 'opacity-60')}
              >
                {spinning ? 'Trekker…' : 'Trekk! 🎲'}
              </button>
            ) : (
              <div className="flex w-full flex-col items-center gap-4 text-center">
                <p className="text-xs font-bold uppercase tracking-widest text-white/60">Trukket!</p>
