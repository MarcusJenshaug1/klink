...
export function ReaksjonsCard({ card, pack, players, intensitet, korttyper, onNext }: ReaksjonsCardProps) {
  ...
  // Move to result when both have tapped
  useEffect(() => {
    ...
    >
      ...
      {(phase === 'waiting' || phase === 'go') && (
        <div className="fixed inset-0 z-40 flex">
          ...
          <button
            ...
            disabled={p2TapMs !== null}
            className={cn(
              'flex flex-1 flex-col items-center justify-center gap-4 select-none transition-colors duration-300 active:brightness-110',
              phase === 'go'
                ? p2TapMs !== null ? 'bg-green-700' : 'bg-green-500'
                : 'bg-red-600'
            )}
          >
            <p className={cn(
              'text-6xl transition-all duration-200 landscape:text-5xl',
              p2TapMs !== null ? 'scale-110' : phase === 'waiting' ? 'opacity-50' : 'animate-pulse'
            )}>
              {p2TapMs !== null ? '✓' : phase === 'go' ? '⚡' : '🔴'}
            </p>
            <p className="text-2xl font-black text-white landscape:text-xl">{p2}</p>
            <p className={cn(
              'text-base font-bold uppercase tracking-wider landscape:text-sm',
              p2TapMs !== null ? 'text-white/60'
              : phase === 'go' ? 'animate-pulse text-white'
              : 'text-white/40'
            )}>
              {p2TapMs !== null ? 'Trykket!' : phase === 'go' ? 'TRYKK!' : 'Vent…'}
            </p>
          </button>
        </div>
      )}

      {/* Card layout (ready + result phases) */}
      {(phase === 'ready' || phase === 'result') && (
        <div className="flex min-h-0 w-full max-w-sm flex-col items-center gap-3 md:max-w-xl md:gap-5 lg:max-w-2xl xl:max-w-3xl landscape:max-w-2xl landscape:gap-2 lg:landscape:max-w-3xl">
          ...
          <div
            className={cn(
              'relative flex max-h-[calc(100dvh-13.5rem)] w-full flex-col overflow-hidden rounded-3xl border shadow-2xl backdrop-blur-md landscape:max-h-[calc(100dvh-8.5rem)] landscape:rounded-2xl',
          ...
          >
            ...
            {phase === 'ready' && (
              <div className="relative z-10 flex flex-col items-center gap-5 p-6 sm:p-7 md:p-10 landscape:p-5">
                ...
                <div className="flex w-full items-center justify-center gap-3">
                  <span className={cn(
                    'flex-1 truncate rounded-2xl px-3 py-2.5 text-center text-sm font-black text-white',
                ...
                <p className="text-center text-sm text-white/70">
                  Begge legger en finger over sin side av skjermen. Trykk så fort det blir grønt!
                </p>
                <button onClick={startCountdown} className={primaryActionClass}>
                  Gjør klar →
                </button>
              </div>
            )}
              ...
              <div className="relative z-10 flex flex-col items-center gap-5 p-6 sm:p-7 md:p-10 landscape:p-5 text-center">
                ...
                <div className="flex w-full gap-3">
                  ...
                  <div className="flex flex-1 flex-col items-center gap-2 rounded-2xl border border-green-400/40 bg-green-500/25 p-4">
                    ...
                    <p className="text-xs text-white/50">{((winnerMs ?? 0) / 1000).toFixed(3)}s</p>
                  ...
                  <div className="flex flex-1 flex-col items-center gap-2 rounded-2xl border border-red-400/30 bg-red-500/20 p-4">
                    ...
                    <p className="text-xs text-white/50">{((loserMs ?? 0) / 1000).toFixed(3)}s</p>
