  ...
  if (packs.length === 0) {
    return (
      <div className={cn('text-center py-10 rounded-3xl px-6', athina ? 'bg-white/18 text-white' : 'bg-white/50')}>
        ...
        <p className={cn('font-bold', athina ? 'text-white' : 'text-forest')}>Ingen pakker tilgjengelig</p>
        <p className={cn('text-sm mt-1', athina ? 'text-white/70' : 'text-forest/60')}>
          Sjekk nettet eller prøv igjen senere.
        </p>
      </div>
    )
  }
    ...
    <div className="grid grid-cols-1 min-[380px]:grid-cols-2 gap-3 sm:gap-4">
      {packs.map((pack) => (
        <PackCard
          ...
          blocked={blockedIds?.has(pack.id)}
          selectedDroyhet={selectedDroyhet}
